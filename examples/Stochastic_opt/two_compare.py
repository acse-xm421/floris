...
figpath = "examples/test_pic/test_exp/layout/farms_"#angle_variance_wd/
csv_file_path = 'exp4-layout.csv'
length = 2000.0
...
def new_layout(distance, variance_wd):
    if variance_wd == 0 or variance_wd == 15:
        ...
        y = np.append([distance+0.5*length,],np.repeat(distance+length, 8))
        y = np.append(y, [distance+0.5*length])
    elif variance_wd == 45:
...
def run_farm(exp_t, mean_wd, variance_wd, distance, length, angle=90, mean_ws=8.0, variance_ws=0.0, solver = "SLSQP"):
    ...
    fixed_layout = 0
    fi_fixed, _, _ = load_fixed_farm(distance, fixed_layout)
...
def main():
    ...
    distance_range = [2600, 3000] #np.linspace(length, length*2, 11)#np.arange(length, length*2+100, 200) #  
    repeat_times = 3
    ...
    for distance in distance_range:
        for angle in angle_range:
            ...
            # i, mean_wd, variance_wd, distance, angle, length = params
                local_params.append((i, 0, variance_wd_range[rank], distance, angle, length))
                i += 1
