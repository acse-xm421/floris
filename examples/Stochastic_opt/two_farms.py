...
from floris.tools.optimization.layout_optimization.layout_optimization_farms import (
    LayoutOptimizationFarms
)
...
if __name__ == "__main__":
    ...
    fi_list = [fi, fi_WR] # independent
    nturbs_list = [fi.floris.farm.n_turbines, fi_WR.floris.farm.n_turbines]
    angle_list = [0,0]
    distance_list = [0, 1000]# if [0,0], overlap, if [0,>1000?], no overlap
    turbine_weights_list = [turbine_weights, turbine_weights]#?

    plot_horizontal(fi_WR, show_wd, "before_opt", yaw_angles=yaw_angles)

    # Setup the optimization problem
    wf_layout_opt = LayoutOptimizationFarms(2, fi_list = fi_list, nturbs_list=nturbs_list,\
                            angle_list=angle_list, distance_list=distance_list)
    ...
    wf_layout_opt.wf.calculate_wake()
    base_aep = wf_layout_opt.get_farm_AEP(freq=freq, turbine_weights=turbine_weights) / 1e6
    wf_layout_opt.reinitialize(layout_x=sol[0], layout_y=sol[1])

    # fi_WR.reinitialize(layout_x=sol_value_x, layout_y=sol_value_y)

    plot_horizontal(wf_layout_opt, show_wd, "after_opt", yaw_angles=yaw_angles)

    wf_layout_opt.calculate_wake()
    turbine_powers = wf_layout_opt.get_turbine_powers()
    farm_power = wf_layout_opt.get_farm_power()
    opt_aep = wf_layout_opt.get_farm_AEP(freq=freq, turbine_weights=turbine_weights) / 1e6
    ...
    wf_layout_opt.plot_layout_opt_results()
